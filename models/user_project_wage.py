...
class UserProjectWage:
    ...
    def __init__(self, user_id, project_id, no_of_days_worked, wage, attendance):
        ...
        self.is_gpm_wage_approved = False
    ...
    def add_user(self, conn):
        try:
            ...
            sql_params = (self.username, self.password, self.email, self.first_name, self.last_name, self.age, self.gender,
                          ...
                          self.is_deleted, )
            cursor_obj = User.query_runner(conn, sql_query, sql_params)
            ...
            elif Login.logged_in_user['role'] == 'gpm' and self.role == 'member':
                sql_query_rel = '''INSERT INTO gpm_mem_rel(gpm_user_id, mem_user_id) VALUES(?,?)'''
            cursor_obj_rel = User.query_runner(conn, sql_query_rel, sql_params_rel)
    ...
    def view_specific_users(conn):
        ...
        if Login.logged_in_user['role'] == 'bdo':
            ...
            cursor_obj_mem = User.query_runner(conn, sql_query_mem, sql_params=None)
            mem_user_ids = [user['mem_user_id'] for user in cursor_obj_mem.fetchall()]
            cursor_obj_mem.close()
            user_ids = set().union(gpm_user_ids, mem_user_ids)
        elif Login.logged_in_user['role'] == 'gpm':
            user_ids = [user['mem_user_id'] for user in cursor_obj.fetchall()]
        cursor_obj.close()
        if user_ids is not None:
            sql_query_user = f'''SELECT user_id, username, role FROM user WHERE user_id IN {str(tuple(user_ids))} AND is_deleted=False'''
            cursor_obj_user = User.query_runner(conn, sql_query_user, sql_params=None)
    ...
    @staticmethod
    def view_user_details(conn, username):
